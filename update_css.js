...
// The replacement theme (Violet from Shadcn)
const newTheme = `@layer base {
  :root {
    --background: 0 0% 100%;
...
// Find the start of the first `@layer base` that contains `:root`
const startRegex = /@layer base\s*\{\s*:root/;
...
if (startIndex !== -1) {
  ...
  if (endIndex !== -1) {
    const newCss = css.substring(0, startIndex) + newTheme + css.substring(endIndex);
